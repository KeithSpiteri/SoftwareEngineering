...
public class Player 
{
	...
	boolean move(char direction)
	{
		if(Character.toUpperCase(direction) == 'U')
		{
			...
			if(setPosition(newP))
			{
				...
				System.out.println("Movement not Allowed. Choose another direction ");
				return false;
			}
		}
	...
	{
		if(p.x >= Map.grid.length || p.y >= Map.grid.length || p.x < 0 || p.y < 0)
			return false;
		else if(Map.grid[p.x][p.y] == Color.BLUE)
		{
			System.out.println("Your just drowned... Bottom line, you have to start from the beginning\n");
